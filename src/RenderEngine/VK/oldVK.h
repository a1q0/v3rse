...
//        createTextureImage();
    ...
    void pickPhysicalDevice() {
        vector<VkPhysicalDevice> devices = enumeratePhysicalDevices();
  ...
  void createLogicalDevice() {
        ...
        vkGetDeviceQueue(vkDevice, indices.graphics.value(), 0, &vkGraphicsQueue);
        vkGetDeviceQueue(vkDevice, indices.present.value(), 0, &vkPresentQueue);
    }

    void createSwapChain() {
        SwapChainSupportDetails swapChainSupport = querySwapChainSupport(vkPhysicalDevice);

        VkSurfaceFormatKHR surfaceFormat = chooseSwapSurfaceFormat(swapChainSupport.vkSurfaceFormats);
        VkPresentModeKHR presentMode = chooseSwapPresentMode(swapChainSupport.vkPresentModes);
        VkExtent2D extent = chooseSwapExtent(swapChainSupport.vkSurfaceCapabilities);

        uint32_t imageCount = swapChainSupport.vkSurfaceCapabilities.minImageCount + 1;
        if (swapChainSupport.vkSurfaceCapabilities.maxImageCount > 0 && imageCount > swapChainSupport.vkSurfaceCapabilities.maxImageCount) {
            imageCount = swapChainSupport.vkSurfaceCapabilities.maxImageCount;
        }
        ...
        createInfo.preTransform = swapChainSupport.vkSurfaceCapabilities.currentTransform;
...
//    }
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, vkPipelineLayout, 0, 1, &descriptorSets[currentFrame], 0, nullptr);
    ...
    bool isDeviceSuitable(VkPhysicalDevice device) {
        ...
        if (extensionsSupported) {
            ...
            swapChainAdequate = !swapChainSupport.vkSurfaceFormats.empty() && !swapChainSupport.vkPresentModes.empty();
        }
